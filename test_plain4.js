function plainToSentences(text){
  ...
  const isClauseStart = (w, next) => {
    ...
    if (/^(и|но|а|или|зато|однако)$/.test(low) && cur.length >= 2) return true;
    if (/^(что|как|где|когда|почему|зачем|куда|ли)$/.test(low) && cur.length >= 2) return true;
    if (/^(да|нет|ну|ладно|хорошо|давай|пожалуйста|вот)$/.test(low) && cur.length >= 1) return true;
    if (/^(чтобы|если|хотя|потому|поэтому|так)$/.test(low) && cur.length >= 2) return true;
    if (/^(я|ты|мы|вы|они|он|она|оно)$/.test(low) && next && /(?:ет|ут|ют|ю|ёшь|ёте|ешь|ете|ется|ются|ал|ла|ло|ли|ть|ти|ться)$/i.test(next)) return true;
    return false;
  };
  const MAXW = 18;
